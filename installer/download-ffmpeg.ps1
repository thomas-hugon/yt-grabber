...
$probe = Join-Path $AppDir "ffprobe.exe"
...
$ffmpegFound = Get-ChildItem $tmp -Filter "ffmpeg.exe" -Recurse | Select-Object -First 1
if (-not $ffmpegFound) { throw "ffmpeg.exe not found in extracted archive" }

$ffprobeFound = Get-ChildItem $tmp -Filter "ffprobe.exe" -Recurse | Select-Object -First 1
if (-not $ffprobeFound) { throw "ffprobe.exe not found in extracted archive" }

Copy-Item $ffmpegFound.FullName $exe -Force
Copy-Item $ffprobeFound.FullName $probe -Force
