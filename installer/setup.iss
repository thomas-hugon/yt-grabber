...
[Run]
Filename: "powershell.exe"; Parameters: "-NoProfile -ExecutionPolicy Bypass -Command ""Invoke-WebRequest -Uri 'https://github.com/yt-dlp/yt-dlp/releases/latest/download/yt-dlp.exe' -OutFile '{app}\yt-dlp.exe' -UseBasicParsing"""; StatusMsg: "Téléchargement de yt-dlp..."; Flags: runhidden waituntilterminated
Filename: "powershell.exe"; Parameters: "-NoProfile -ExecutionPolicy Bypass -Command ""$zip='{app}\ffmpeg.zip'; Invoke-WebRequest -Uri 'https://github.com/BtbN/FFmpeg-Builds/releases/download/latest/ffmpeg-master-latest-win64-lgpl-essentials.zip' -OutFile $zip -UseBasicParsing; Expand-Archive -Path $zip -DestinationPath '{app}\ffmpeg_tmp' -Force; $f=Get-ChildItem '{app}\ffmpeg_tmp' -Filter 'ffmpeg.exe' -Recurse | Select-Object -First 1; Copy-Item $f.FullName '{app}\ffmpeg.exe'; Remove-Item $zip -Force; Remove-Item '{app}\ffmpeg_tmp' -Recurse -Force"""; StatusMsg: "Téléchargement de ffmpeg..."; Flags: runhidden waituntilterminated
